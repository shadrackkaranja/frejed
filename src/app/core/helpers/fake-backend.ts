...
export class fakebackendInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        return of(null).pipe(mergeMap(() => {
            ...
            // get asset list
            if (request.url.endsWith('/app/assetList') && request.method === 'GET') {
                if (assetList) {
                    return of(new HttpResponse({ status: 200, body: assetList }));
                } else {
                    return throwError({ status: 401, error: { message: 'No Data Found' } });
                }
            }
            // Add asset list
            if (request.url.endsWith('/app/assetList') && request.method === 'POST') {
                const newUser = request.body;
                if (assetList) {
                    return of(new HttpResponse({ status: 200, body: newUser }));
                } else {
            ...
            // Update asset list
            if (request.url.endsWith('/app/assetList') && request.method === 'PUT') {
                const updatedUser = request.body;
                if (assetList) {
                    return of(new HttpResponse({ status: 200, body: updatedUser }));
                } else {
            ...
            // DELETE asset list
            if (request.url.endsWith('/app/assetList') && request.method === 'DELETE') {
                const updatedUser = request.body;
                if (assetList) {
                    return of(new HttpResponse({ status: 200, body: updatedUser })); // respond 200 OK
                } else {
             ...
             // get cash list
             if (request.url.endsWith('/app/cashEquivalentList') && request.method === 'GET') {
                if (cashEquivalentList) {
                    return of(new HttpResponse({ status: 200, body: cashEquivalentList }));
                } else {
                    return throwError({ status: 401, error: { message: 'No Data Found' } });
                }
            }
            // Add cash list
            if (request.url.endsWith('/app/cashEquivalentList') && request.method === 'POST') {
                const newUser = request.body;
                if (cashEquivalentList) {
                    return of(new HttpResponse({ status: 200, body: newUser }));
                } else {
            ...
            // Update cash list
            if (request.url.endsWith('/app/cashEquivalentList') && request.method === 'PUT') {
                const updatedUser = request.body;
                if (cashEquivalentList) {
                    return of(new HttpResponse({ status: 200, body: updatedUser }));
                } else {
            ...
            if (request.url.endsWith('/app/equityList') && request.method === 'DELETE') {
             ...
             // get expenses list
             if (request.url.endsWith('/app/expensesList') && request.method === 'GET') {
                if (expensesList) {
                    return of(new HttpResponse({ status: 200, body: expensesList }));
                } else {
                    return throwError({ status: 401, error: { message: 'No Data Found' } });
                }
            }
            // Add expenses list
            if (request.url.endsWith('/app/expensesList') && request.method === 'POST') {
                const newUser = request.body;
                if (expensesList) {
                    return of(new HttpResponse({ status: 200, body: newUser }));
                } else {
            ...
            if (request.url.endsWith('/app/expensesList') && request.method === 'PUT') {
                const updatedUser = request.body;
                if (expensesList) {
                    return of(new HttpResponse({ status: 200, body: updatedUser }));
                } else {
            ...
            // get recievablesList
            if (request.url.endsWith('/app/recievablesList') && request.method === 'GET') {
                if (recievablesList) {
                    return of(new HttpResponse({ status: 200, body: recievablesList }));
                } else {
                    return throwError({ status: 401, error: { message: 'No Data Found' } });
                }
            }
            // Add recievablesList list
            if (request.url.endsWith('/app/recievablesList') && request.method === 'POST') {
                const newUser = request.body;
                if (recievablesList) {
                    return of(new HttpResponse({ status: 200, body: newUser }));
                } else {
            ...
            // Update recievablesList list
            if (request.url.endsWith('/app/recievablesList') && request.method === 'PUT') {
                const updatedUser = request.body;
                if (recievablesList) {
                    return of(new HttpResponse({ status: 200, body: updatedUser }));
                } else {
            ...
            // DELETE recievablesList list
            if (request.url.endsWith('/app/recievablesList') && request.method === 'DELETE') {
                const updatedUser = request.body;
                if (recievablesList) {
                    return of(new HttpResponse({ status: 200, body: updatedUser })); // respond 200 OK
                } else {
            ...
            // get revenue list
            if (request.url.endsWith('/app/revenueList') && request.method === 'GET') {
                if (revenueList) {
                    return of(new HttpResponse({ status: 200, body: revenueList }));
                } else {
                    return throwError({ status: 401, error: { message: 'No Data Found' } });
                }
            }
            // Add revenue list
            if (request.url.endsWith('/app/revenueList') && request.method === 'POST') {
                const newUser = request.body;
                if (revenueList) {
                    return of(new HttpResponse({ status: 200, body: newUser }));
                } else {
            ...
            // Update revenue list
            if (request.url.endsWith('/app/revenueList') && request.method === 'PUT') {
                const updatedUser = request.body;
                if (revenueList) {
                    return of(new HttpResponse({ status: 200, body: updatedUser }));
                } else {
            ...
            // DELETE revenue list
            if (request.url.endsWith('/app/revenueList') && request.method === 'DELETE') {
                const updatedUser = request.body;
                if (revenueList) {
                    return of(new HttpResponse({ status: 200, body: updatedUser })); // respond 200 OK
                } else {
            ...
            // get taxation list
            if (request.url.endsWith('/app/taxationList') && request.method === 'GET') {
                if (taxationList) {
                    return of(new HttpResponse({ status: 200, body: taxationList }));
                } else {
                    return throwError({ status: 401, error: { message: 'No Data Found' } });
                }
            }
            // Add taxation list
            if (request.url.endsWith('/app/taxationList') && request.method === 'POST') {
                const newUser = request.body;
                if (taxationList) {
                    return of(new HttpResponse({ status: 200, body: newUser }));
                } else {
            ...
            // Update taxation list
            if (request.url.endsWith('/app/taxationList') && request.method === 'PUT') {
                const updatedUser = request.body;
                if (taxationList) {
                    return of(new HttpResponse({ status: 200, body: updatedUser }));
                } else {
            ...
            // DELETE taxation list
            if (request.url.endsWith('/app/taxationList') && request.method === 'DELETE') {
                const updatedUser = request.body;
                if (taxationList) {
                    return of(new HttpResponse({ status: 200, body: updatedUser })); // respond 200 OK
                } else {
